...
Color Red             = Color(0, 102, 255, 255);
Color inactive          = Color(255, 255, 255, 255);
Color MainColor         = Red;

Color FontColor         = Color(255, 255, 255, 255);
Color background        = Color(0, 0, 0, 255);
Color border1           = Color(0, 0, 0, 255);
Color border2           = Color(0, 102, 255, 255);
...
void cMenu::renderSlider(int x, int y, int w, const char* szString, int& value, int max, int min) {
    ...
    draw->fillrgba(x, y + 5, w + 2, h, Color(0, 0, 0, 255));
...
void cMenu::renderCombo(int x, int y, int w, int h, const char* szString, vector<string> szValue, int& currValue, bool* bOpend) {
    ...
    Color col1  = Color(0, 102, 255, 255);
    Color col2  = Color(0, 0, 0, 255);
    ...
    if (*bOpend) {
        ...
        for ( int i = 0; i < szValue.size(); i++ ) {
            ...
            draw->drawstring(x + 10, Y + 3 + (i * h), Color(255, 255, 255, 255), mFont, szValue[i].c_str());
    ...
    // Draws the box
    draw->fillrgba(x, y, w, h, Color(0, 0, 0, 255));
...
void cMenu::drawcolorpicker(int x, int y, const char *szString, Color &col) {
    ...
    draw->fillrgba(x, y + 0, 0, 0, Color(col.r(), col.g(), col.b(), 255));
    draw->drawbox(x, y + 255, 0, 0, Color::Red  ());
...
void cMenu::renderAim(int x, int y) {
    ...
    this->renderCheckbox(x, y + 90, "Autowall", &vars.aimbot.autowall);
    this->renderSlider(x, y + 100, 180, "", vars.aimbot.mindmg, 100, 0);
    this->renderCheckbox(x, y + 120, "Min Dmg", &vars.aimbot.mindamage);
    this->renderSlider(x, y + 130, 180, "", vars.aimbot.mindmg, 80, 0);
    this->renderCheckbox(x, y + 150, "Hitchance", &vars.aimbot.hitchance);
    this->renderSlider(x, y + 160, 180, "", vars.aimbot.accuracyhithcance, 100, 0);
    draw->drawstring(x, y + 180, FontColor, mFont, "Pointscale");
    this->renderSlider(x, y + 190, 180, "", vars.aimbot.pointscale, 100, 0);
    
    vector<string> Resolve;  // Resolver
    
    Resolve.push_back("Off");
    Resolve.push_back("Revolution");
    Resolve.push_back("Delta");
    Resolve.push_back("pnegative100");
    Resolve.push_back("Test");
    Resolve.push_back("TestV2");
    Resolve.push_back("LBY Resolver");
    
    this->renderCheckbox(x, y + 210, "Yaw Resolver", &vars.aimbot.Yawresolver);
    this->renderCombo(x, y + 220, 110, 20, "Off", Resolve, vars.aimbot.yresolve, &vars.resolver_opend); // 150
    ...
    this->renderCheckbox(x + 200, y + 60, "baim", &vars.aimbot.autostop);
    this->renderCheckbox(x + 200, y + 130, "Prediction", &vars.aimbot.prediction);
    this->renderCheckbox(x + 200, y + 150, "Auto Zeus", &vars.aimbot.autozeus);
    this->renderCheckbox(x + 200, y + 170, "Freestanding", &vars.aimbot.freestanding);
    
    vector<string> hitscan;  // Hitscan types
    
    hitscan.push_back("Off");
    hitscan.push_back("Default");
    ...
    this->renderCheckbox(x + 310, y, "AntiAim Enabled", &vars.misc.antiaim);
    this->renderCheckbox(x + 310, y + 20, "Show Real Angles", &vars.misc.thirdpersonmode);
...
void cMenu::renderVis(int x, int y) {
    ...
    this->renderCheckbox(x, y + 120, "Heath text", &vars.visuals.healthtext);
    this->renderCheckbox(x, y + 140, "Skeleton", &vars.visuals.skeleton);
    this->renderCheckbox(x, y + 160, "Snaplines", &vars.visuals.snapline);
    this->renderCheckbox(x, y + 180, "Grenade ESP", &vars.visuals.grenade);
    this->renderCheckbox(x, y + 200, "Armor", &vars.visuals.armour);
    this->renderCheckbox(x, y + 220, "Active Weapon", &vars.visuals.activeweapon);
    ...
    this->renderCheckbox(x + 325, y, "Player Chams", &vars.visuals.chams);
    this->renderSlider(x + 325, y + 20, 170, "", vars.visuals.playerchams_alpha, 255, 0);
    this->renderCheckbox(x + 325, y + 40, "Hand Chams", &vars.visuals.handchams);
    this->renderSlider(x + 325, y + 60, 170, "", vars.visuals.handchams_alpha, 255, 0);
    this->renderCheckbox(x + 325, y + 80, "Weapon Chams", &vars.visuals.weaponchams);
    this->renderSlider(x + 325, y + 100, 170, "", vars.visuals.weaponchams_alpha, 255, 0);
    if(vars.visuals.chams) {
    this->renderCombo(x + 325, y + 120, 125, 20, "Lit", Players, vars.visuals.playersType, &vars.players_opend);
    }
    if(vars.visuals.handchams) {
    if(!vars.players_opend)
    this->renderCombo(x + 325, y + 140, 125, 20, "Lit", Hands, vars.visuals.handsType, &vars.hands_opend);
    }
    if(vars.visuals.weaponchams) {
    if((!vars.players_opend) && !vars.hands_opend)
    this->renderCombo(x + 325, y + 160  , 125, 20, "Lit", Weapons, vars.visuals.weaponType, &vars.weapons_opend);
    }
}
...
void cMenu::renderConfigs(int x, int y) {
    ...
    conf.push_back("Auto");
    conf.push_back("Sniper");
    conf.push_back("Pistol");
...
void cMenu::renderMisc(int x, int y) {
    ...
    this->renderSlider(x, y + 70, 180, "FOV", vars.misc.fov, 80, 0); // pushed fov here
    this->renderCheckbox(x, y + 90, "No recoil", &vars.misc.norecoil);
    this->renderCheckbox(x, y + 110, "No visual recoil", &vars.misc.novisual);
    this->renderCheckbox(x, y + 130, "No Scope", &vars.misc.noscope);
    this->renderCheckbox(x, y + 150, "Chat Spam", &vars.misc.spammer);
   // this->renderCheckbox(x, y + 170, "mac.pub", &vars.misc.macpub);
    this->renderCheckbox(x, y + 170, "Insane Mode", &vars.misc.gamesense);
  //  this->renderCheckbox(x, y + 210, "Skeet.cc", &vars.misc.skeet.cc);
    this->renderCheckbox(x, y + 190, "Watermark", &vars.misc.watermark);
    this->renderCheckbox(x, y + 210, "Show ranks", &vars.misc.showrank);
    ...
    this->renderCheckbox(x + 300, y + 20, "Night Mode", &vars.misc.nightmode);
    ...
    this->renderCheckbox(x + 300, y + 100, "Asuswall", &vars.misc.asuswalls);
    this->renderCheckbox(x + 300, y + 120, "FakeWalk", &vars.misc.fakewalk);
   // this->renderCheckbox(x + 300, y + 140, "MoonWalk", &vars.misc.moon);
    //this->renderCheckbox(x + 300, y + 160, "Knife Bot", &vars.misc.knifebot);
    
    vector<string> FakeWalk;
...
void cMenu::renderMenu() {
    ...
    draw->RectOutlined(x, y, w, h, 1, Color(0, 0, 0, 255), Color(0, 102, 255, 255));
    draw->fillrgba(x + 2, y + 2, w - 4, 20, Color(0, 0, 0, 255));
    draw->drawstring(x + ( ( w - 4 ) / 2 ) + 2, y + 10, Color::White(), mFont, "skeetformac", true);
    
    // Draws tabs
    draw->RectOutlined(x + 2, y + 22 + 5, 96, 20, 1, Color(0, 0, 0, 255), Color::Black());
    draw->RectOutlined(x + 102, y + 22 + 5, 96, 20, 1, Color(0, 0, 0, 255), Color::Black());
    draw->RectOutlined(x + 202, y + 22 + 5, 96, 20, 1, Color(0, 0, 0, 255), Color::Black());
    draw->RectOutlined(x + 302, y + 22 + 5, 96, 20, 1, Color(0, 0, 0, 255), Color::Black());
    draw->RectOutlined(x + 402, y + 22 + 5, 96, 20, 1, Color(0, 0, 0, 255), Color::Black());
    
    
    // Handles tabs
    if(draw->inArea(x + 2, y + 27, 96, 20)) {
        
        draw->RectOutlined(x + 2, y + 22 + 5, 96, 20, 1, Color(0, 102, 255, 255), Color::Black());
        ...
        draw->RectOutlined(x + 102, y + 22 + 5, 96, 20, 1, Color(0, 102, 255, 255), Color::Black());
        ...
        draw->RectOutlined(x + 202, y + 22 + 5, 96, 20, 1, Color(0, 102, 255, 255), Color::Black());
        ...
        draw->RectOutlined(x + 302, y + 22 + 5, 96, 20, 1, Color(0, 102, 255, 255), Color::Black());
        ...
        draw->RectOutlined(x + 402, y + 22 + 5, 96, 20, 1, Color(0, 102, 255, 255), Color::Black());
    ...
    switch (curTab) {
        ...
        case mTab::RageTab:
            draw->RectOutlined(x + 2, y + 22 + 5, 96, 20, 1, Color(0, 0, 0, 255), Color(0, 0, 0, 255));
            this->renderAim(x + 2 + 5, y + 27 + 20 + 5);
            break;
            
        case mTab::PlayerTab:
            draw->RectOutlined(x + 102, y + 22 + 5, 96, 20, 1, Color(0, 0, 0, 255), Color(0, 0, 0, 255));
            this->renderPlayer(x + 2 + 5, y + 27 + 20 + 5);
            break;
            
        case mTab::VisTab:
            draw->RectOutlined(x + 202, y + 22 + 5, 96, 20, 1, Color(0, 0, 0, 255), Color(0, 0, 0, 255));
            this->renderVis(x + 2 + 5, y + 27 + 20 + 5);
            break;
            
        case mTab::MiscTab:
            draw->RectOutlined(x + 302, y + 22 + 5, 96, 20, 1, Color(0, 0, 0, 255), Color(0, 0, 0, 255));
            this->renderMisc(x + 2 + 5, y + 27 + 20 + 5);
            break;
            
        case mTab::ColorsTab:
            draw->RectOutlined(x + 402, y + 22 + 5, 96, 20, 1, Color(0, 0, 0, 255), Color(0, 0, 0, 255));
    ...
    draw->drawstring(x + 50, y + 22 + 14, FontColor, mFont,  " A ", true);
    draw->drawstring(x + 150, y + 22 + 14, FontColor, mFont, " P ", true);
    draw->drawstring(x + 250, y + 22 + 14, FontColor, mFont, " V ", true);
    draw->drawstring(x + 350, y + 22 + 14, FontColor, mFont, " M ", true);
    draw->drawstring(x + 450, y + 22 + 14, FontColor, mFont, " C ", true);
